...
export function useVapi(book: IBook) {
        ...
        const handlers = {
            ...
            'call-end': () => {
                ...
                if (sessionIdRef.current) {
                    endSessionMutation({ sessionId: sessionIdRef.current as any, durationSeconds: durationRef.current }).catch((err: Error) =>
    ...
    const start = useCallback(async () => {
        ...
        try {
            ...
            const billingPeriodStart = new Date(new Date().getFullYear(), new Date().getMonth(), 1).getTime();
